...
// JWT key
export async function loginUserToken(user) {

  const endpoint = 'https://localhost:7121/api/User/login/user';
  const config = {
    headers: {
  ...
  const res = await axios.post(endpoint, JSON.stringify(user), config);
